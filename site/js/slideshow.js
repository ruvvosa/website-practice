...
var current = 0;  // 슬라이드 번호르 저장할 인덱스 변수 초깃값 0
...
function prevSlide(){
    ...
    else current = slides.length - 1; // 아니면 마지막 위치로
        showSlides(current); //이미지 표시
}

function nextSlide(){
    if(current < slides.length -1) current += 1; //2보다 작으면 다음 위치로
    else current = 0; //이미지가 마지막이면 첫 번째 위치로 이동
        showSlides(current);
}
